...
const Account = ({getCurrentProfile, auth: {user}, profile: {profile, loading}}) => {
    ...
    return (
        user && loading ? <div>loading...</div> : <Fragment>
            ...
            <div className="avatarField" ><Fragment><div className="avatar-update"><img onClick={e=> setPictureScreen(!pictureScreen)} src={require("./style/uploadPhoto.png")} height="24px" onClick={e=>setImageView(!imageView)} /></div>{user && user.avatar.charAt(0) == "/" ? <img onClick={e=> setPictureScreen(!pictureScreen)} src={user.avatar} height="100%" /> : <img onClick={e=> setPictureScreen(!pictureScreen)} src={`https://s3.eu-west-2.amazonaws.com/onloud-storage/profile/avatar/${user.avatar}`} height="100%" />}</Fragment></div>
            {
